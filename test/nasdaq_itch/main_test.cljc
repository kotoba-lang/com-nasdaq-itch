(ns nasdaq_itch.main-test
  "Contract + behavioral test for the nasdaq_itch-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "nasdaq_itch-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'nasdaq_itch.main-test)]
