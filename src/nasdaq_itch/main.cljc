(ns nasdaq_itch.main
  "Kotodama WASM entrypoint for the Nasdaq_itch clean-room actor (L4) — Clojure port.
  ...
  facts (`nasdaq_itch.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "nasdaq_itch")
(def tier "L4")
...
(def entity-specs
  [{:entity "Instrument" :plural "instruments" :id-prefix "nasdaqit_ins"
   ...
   {:entity "Execution"  :plural "executions"  :id-prefix "nasdaqit_exe"
    :fields [:orderId :quantity :price :executedAt]    :required [:quantity :price]
    :coerce {:quantity :float :price :float}           :refs {:orderId "Order"}}
   {:entity "MarketData" :plural "marketdatas" :id-prefix "nasdaqit_mar"
   ...
   {:entity "Position"   :plural "positions"   :id-prefix "nasdaqit_pos"
...
(defn emit-facts
  "EAVT facts for one record: {\"nasdaq_itch.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "nasdaq_itch-compat" :tier tier :entities entities} 200])
...
(defn start! [] :nasdaq_itch-compat/ready)
